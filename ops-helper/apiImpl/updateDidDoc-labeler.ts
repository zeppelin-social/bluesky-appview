...
const main = async function (opt: any) {
  ...
  const credentials = {
    ...gotCreds.data,
    verificationMethods: {
      ...gotCreds.data.verificationMethods,
      atproto_label: skey.did(),
    },
    services: {
      ...gotCreds.data.services,
      atproto_labeler: { type: "AtprotoLabeler", endpoint: opt.labelerURL },
    },
...
const opt = yargs(process.argv.slice(2))
  .options({
    labelerURL: {
      type: "string",
      description: "labeler URL",
      default: `https://ozone.${dom}`,
    },
    pdsURL: {
      type: "string",
      description: "PDS URL",
    ...
    plcSignToken: {
      type: "string",
      description: "PLC Token received in Email",
    },
    tls: {
      type: "string",
