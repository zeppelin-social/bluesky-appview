...
const main = async function (opt: any) {
  ...
  const resp = await agent.api.tools.ozone.moderation.emitEvent(ev, {
    encoding: "application/json",
    headers: { "atproto-proxy": `${agent.session!.did}#atproto_labeler` },
  });
...
const opt = yargs(process.argv.slice(2))
  .options({
    ...
    comment: {
      type: "string",
      description: "comment on assigning label",
      default: "",
    },
    tls: {
      type: "string",
